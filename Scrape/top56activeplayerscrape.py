...
def top56activeplayerscrape():
    # Cloud MySQl database connection on AWS
    engine = create_engine(f"mysql://{remote_gwsis_dbuser}:{remote_gwsis_dbpwd}@{remote_db_endpoint}:{remote_db_port}/{remote_gwsis_dbname}")

    #Active 3 scorers
    scorerid = [8466139, 8471214, 8471675]

    #creates the gamelog df
    df = pd.DataFrame(columns=['id','firstname','lastname','pos','birthdate','daysalive','season','seasonid','assists','goals','pim','shots','games','hits','ppg','ppp','gwg','otg','shg','shp','bs','pm','pts','team','opp','date','home','win'])
    ...
    df.to_sql(name='top56_scorers_GL_current_data', if_exists='replace', con=conn, chunksize=500, index=False)

    #create historical data table & then combine historical and current data
    df_hist = pd.read_sql('SELECT * FROM top56_scorers_GL_hist_data', con=engine)
